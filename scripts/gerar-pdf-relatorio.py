...
DOCS = ROOT / "docs"
OUTPUT = DOCS / "pdf" / "relatorio-tecnico.pdf"
...
def main():
    ...
    story.extend(markdown_to_flowables(DOCS / "relatorio-tecnico.md", styles))
    story.append(PageBreak())
    story.append(Paragraph("Verificação Técnica", styles["Heading1"]))
    story.extend(markdown_to_flowables(DOCS / "verificacao-tecnica.md", styles))
    ...
    doc = SimpleDocTemplate(
        ...
        topMargin=1.6 * cm,
        bottomMargin=1.5 * cm,
        title="Relatório Técnico - Raízes do Nordeste API",
        author="Raízes do Nordeste API",
    )
