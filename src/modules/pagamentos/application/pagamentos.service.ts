...
export class PagamentosService {
  ...
  async processarMock(pedidoId: number, dto: ProcessarPagamentoMockDto, user: AuthUser) {
    ...
    if (!pedido || !pedido.pagamento) {
      throw new NotFoundException('Pedido ou pagamento não encontrado.');
    }

    this.validarPosseCliente(pedido.cliente.usuarioId, user);

    if (pedido.status !== PedidoStatus.AGUARDANDO_PAGAMENTO) {
      throw new ConflictException('Pedido não aceita pagamento neste status.');
    }

    // Pagamento mock é processado uma única vez para evitar duplo crédito ou duplo estorno.
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Pontos só são creditados quando o pagamento fecha e o cliente aceitou fidelidade.
      ...
      if (!aprovado) {
        // Se o pagamento falha, a reserva feita na criação do pedido volta para o estoque.
  ...
  private validarPosseCliente(usuarioId: number, user: AuthUser) {
    if (user.role === Role.CLIENTE && usuarioId !== user.sub) {
      throw new ForbiddenException('Cliente só pode acessar pagamento dos próprios pedidos.');
    }
  }
}
