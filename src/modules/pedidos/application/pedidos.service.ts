...
export class PedidosService {
  ...
  async criar(dto: CreatePedidoDto, user: AuthUser) {
    if (dto.formaPagamento !== FormaPagamento.MOCK) {
      throw new BadRequestException('Apenas pagamento MOCK e permitido neste MVP.');
    }
    ...
    if (!unidade || !unidade.ativa) {
      throw new NotFoundException('Unidade nao encontrada ou inativa.');
    }

    const produtoIds = dto.itens.map((item) => item.produtoId);
    // O cardapio da unidade define se o produto pode ser vendido naquele canal.
    ...
    if (cardapios.length !== produtoIds.length) {
      throw new NotFoundException('Um ou mais produtos nao existem no cardapio da unidade.');
    }
    ...
    const linhas = dto.itens.map((item) => {
      ...
      if (!cardapio || !estoque) {
        throw new NotFoundException('Produto sem cardapio ou estoque na unidade.');
      }
    ...
    // Pedido, pagamento pendente e reserva de estoque nascem juntos para manter consistencia.
  ...
  async buscar(id: number, user: AuthUser) {
    ...
    if (user.role === Role.CLIENTE && pedido.cliente.usuarioId !== user.sub) {
      throw new ForbiddenException('Cliente so pode consultar os proprios pedidos.');
    }
  ...
  async atualizarStatus(id: number, dto: UpdatePedidoStatusDto, user: AuthUser) {
    if (!STATUS_MANUAIS.includes(dto.status)) {
      throw new BadRequestException('Status nao permitido para atualizacao manual.');
    }
  ...
  async cancelar(id: number, user: AuthUser) {
    ...
    if (statusFinais.includes(pedido.status)) {
      throw new ConflictException('Pedido nao pode ser cancelado neste status.');
    }
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // O estoque foi reservado na criacao do pedido e precisa voltar se o pedido nao foi concluido.
  ...
  private async resolveClienteId(dto: CreatePedidoDto, user: AuthUser) {
    if (user.role === Role.CLIENTE) {
      ...
      if (!cliente) {
        throw new NotFoundException('Cliente nao encontrado para usuario autenticado.');
      }
      return cliente.id;
    }

    if (!dto.clienteId) {
      throw new BadRequestException('clienteId e obrigatorio para perfis operacionais.');
    }

    const cliente = await this.prisma.cliente.findUnique({ where: { id: dto.clienteId } });
    if (!cliente) {
      throw new NotFoundException('Cliente informado nao encontrado.');
    }
