...
export class AuthService {
  ...
  async login(dto: LoginDto) {
    ...
    // O token carrega apenas dados necessários para autorização nos guards.
    const payload = { sub: usuario.id, email: usuario.email, role: usuario.role };

    // Login é uma ação sensível, por isso fica registrado para rastreabilidade.
  ...
  async register(dto: RegisterDto) {
    ...
    // Cliente recebe cadastro complementar e saldo inicial dentro da mesma transação.
