...
export class EstoqueService {
  ...
  async buscar(unidadeId: number, produtoId: number) {
    ...
    if (!estoque) {
      throw new NotFoundException('Estoque nao encontrado para unidade e produto.');
    }
  ...
  async movimentar(dto: CreateMovimentoEstoqueDto, usuarioId: number) {
    // A quantidade e o historico de movimento precisam ser gravados juntos.
    return this.prisma.$transaction(async (tx) => {
      ...
      // Saidas manuais nao podem deixar o estoque negativo; isso evita vender item inexistente.
      if (novaQuantidade < 0) {
