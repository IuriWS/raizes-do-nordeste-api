...
A suite e2e usa a API NestJS por HTTP e banco PostgreSQL configurado em `DATABASE_URL`. Rode em um banco descartável de desenvolvimento, pois o teste limpa tabelas antes de cada cenário.
...
- Pagamento mock recusado com restauração de estoque.
- Bloqueio de pagamento de pedido alheio.
- Cancelamento com restauração de estoque e bloqueio de pagamento posterior.
...
- `npm test` com testes unitários passando.
