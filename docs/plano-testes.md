...
| T02 | Acesso sem token | `/pedidos` | GET | API online | Sem header Authorization | 401 com erro padrao | T02 Acesso sem token |
...
| T05 | Criar pedido sem canal | `/pedidos` | POST | Token CLIENTE | Body sem `canalPedido` | 422 com erro padrao | T05 Criar pedido sem canalPedido |
...
| T10A | Preparar pagamento recusado | `/pedidos` | POST | Token CLIENTE e estoque disponivel | Pedido APP separado | 201 com `pedidoRecusadoId` | T10A Preparar pedido para pagamento recusado |
...
| T14 | Cadastro público sem perfil interno | `/auth/register` | POST | API online | Body sem `role` | 201 com usuário CLIENTE | Automatizado e2e |
| T15 | Cadastro publico com perfil interno | `/auth/register` | POST | API online | Body com `role` | 422 com erro padrao | Automatizado e2e |
| T16 | Pagamento de pedido alheio | `/pagamentos/mock/:pedidoId` | POST | Dois clientes cadastrados | Cliente tenta pagar pedido de outro | 403 com erro padrao | Automatizado e2e |
...
| T18 | Pagamento apos cancelamento | `/pagamentos/mock/:pedidoId` | POST | Pedido cancelado | Resultado APROVADO | 409 com erro padrao | Automatizado e2e |
...
A suite `npm run test:e2e` automatiza os principais cenarios do plano:

- T01: login válido.
- T02: acesso sem token.
- T03: perfil sem permissão.
- T04: criação de pedido válido.
- T05: criacao de pedido sem `canalPedido`.
- T09: pagamento mock aprovado.
- T10B: pagamento mock recusado com restauracao de estoque em pedido separado.
- T12: filtro de pedidos por canal.
- T14 e T15: cadastro publico limitado a CLIENTE.
- T16: bloqueio de pagamento de pedido alheio.
- T17 e T18: cancelamento restaura estoque e impede pagamento posterior.

Os demais cenarios continuam documentados para execucao manual via Postman ou futura ampliacao da suite.
